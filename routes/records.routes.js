    ...
    try {
        ...
        const result = await pool.query(
            `SELECT rv.*
             ...
             WHERE rv.question_id = $1 AND r.sdg_id = $2 AND YEAR = $3 AND r.user_id = $4`,
...
router.post("/records-values/check", async (req, res) => {
    try {
        ...
        const result = await pool.query(
            `SELECT r.record_id
             FROM records r
             WHERE r.sdg_id = $1 AND r.year = $2 AND r.user_id = $3`,
        ...
        cloudinary.uploader.upload(req.file.path, async function (err, result) {
            if (err) {
                ...
                try {
                    ...
                    console.log("Received sectionData from client:", result);

                    // Extract the necessary data for the evidence record
                    const evidence = {
                        evidence_id:
                            "EID" + Math.floor(Math.random() * 900000 + 100000),
                        name: result.url, // Use the stored filename
...
router.post(
    ...
    async (req, res) => {
            ...
            if (err) {
                ...
                try {
                    ...
                    const annualReport = {
                        ...
                        file_path: result.url, // Use the stored filename (or path)
                    };
        ...
        `;
                    ...
                    res.status(200).json({
                        message:
                            "Annual report uploaded and data saved successfully!",
                    });
                } catch (error) {
                    console.error("Error uploading annual report:", error);
                    res.status(500).json({
                        message:
                            "Failed to upload annual report and save data.",
                    });
                }
            }
        });
